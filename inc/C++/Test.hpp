...
#define TEST_FORMAT_ALL TEST_FORMAT_MAX | TEST_FORMAT_ERROR | TEST_FORMAT_SUCCESS
        ...
        class Test {
            ...
            inline static size_t nb() {
                trycatch_assert(_launched,
                                debug_elogs("Please run the tests before recovering this value"));
                return _tests.size();
            }
            /*!
            ...
            /*!
             * \brief		Add a function (without arguments and returning void) that contains
             * a group of tests, could be a lambda \param[in]	name		A name to group the
             * tests
             * and to call in run \param[in]	func		The function containing the group of
             * tests \return		void
             */
            inline static void addSuite(const std::string& name, std::function< void() > func) {
                _funcs[name].push_back(func);
            }
            /*!
             ...
             * \return		void
             */
            inline static void addThrow(const std::string& instr,
                                        const std::string& except,
                                        const Test_struct& test) {
                addTest({test.expr, instr + " throws " + except, test.file, test.func, test.line});
            }
            ...
            static void in(const T&           value,
                           const U&           array,
                           const Test_struct& test,
                           bool               start = false) {
                bool isIn = start;

                if (std::find(array.begin(), array.end(), value) != array.end()) { isIn = !isIn; }

                addTest(
                    {isIn, mcd::tos(value) + " in " + test.str, test.file, test.func, test.line});
            }
             ...
             * \param[in]	test	Some test analyze datas
             * \param[in]	cmp		Set if instance is normal or inverted, \b true is
             * normal \return		void
             */
            template< class T, class U >
            static void instance(const U&,
                                 const std::string& strObj,
                                 const Test_struct& test,
                                 bool               cmp = true) {
                addTest({(std::is_base_of< T, U >::value == cmp),
                         std::string("Is ") + strObj + " an instance of " + test.str,
                         test.file,
                         test.func,
                         test.line});
            }
             ...
             * \param[in]		format	The format of the analyze, default is
             * TEST_FORMAT_ALL \return			void
             */
...
#define unit_false(EXPR) TEST_ADD_TEST((!static_cast< bool >(EXPR)), "! #EXPR")

#define unit_equals(A, B) unit_true(A == B)
...
#define unit_throws(INSTR, EXCEPT)                                                                 \
    {                                                                                              \
        ...
        try {                                                                                      \
            INSTR;                                                                                 \
        } catch (const EXCEPT&) { throws = true; } catch (const std::exception&) {                 \
        }                                                                                          \
...
#define unit_noThrows(INSTR, EXCEPT)                                                               \
    {                                                                                              \
        ...
        try {                                                                                      \
            INSTR;                                                                                 \
        } catch (const EXCEPT&) { throws = false; } catch (const std::exception&) {                \
        }                                                                                          \
        TEST_ADD_THROW(INSTR, EXCEPT, throws);                                                     \
    }
