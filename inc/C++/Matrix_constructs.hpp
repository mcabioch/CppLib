...
template< class C >
Matrix< C >::Matrix() : _i(0), _j(0), _datas(), _enabled(false), _square(true) {}

template< class C >
Matrix< C >::Matrix(const std::vector< std::vector< C > >& datas) : Matrix() {
    init(datas);
}

template< class C >
Matrix< C >::Matrix(const size_t& width, const size_t& height) : Matrix() {
    std::vector< std::vector< C > > datas;

    for (size_t i = 0; i < height; ++i) {
        std::vector< C > data;
        for (size_t j = 0; j < width; ++j) { data.push_back(0); }
...
Matrix< C >::Matrix(std::array< std::array< C, M >, N >& datas) : Matrix() {
...
void Matrix< C >::init_vect(size_t i, size_t j, C default_value) {
    ...
    for (size_t index_i = 0; index_i < i; ++index_i) {
        ...
        for (size_t index_j = 0; index_j < j; ++index_j) { data.push_back(default_value); }
...
void Matrix< C >::init(const std::vector< std::vector< C > >& datas) {
    ...
    for (auto line : datas) {
        ...
        if (maxJ < line.size()) {
            ...
            for (size_t i = line.size(); i < maxJ; ++i) { formatted_line.push_back(0); }
        }
    ...
    for (auto& formatted_line : formatted_datas) {
        if (formatted_line.size() < maxJ) {
            for (size_t i = formatted_line.size(); i < maxJ; ++i) { formatted_line.push_back(0); }
        }
    }
