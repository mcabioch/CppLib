...
namespace mcd
{
    ...
    V mod(V a, U b, bool positive = false) {
        V factA;
        U factB;
        ...
        V out = static_cast< V >(static_cast< double >(static_cast< long long int >(a * factA) %
                                                       static_cast< long long int >(b * factB))) /
                static_cast< V >(factA * static_cast< V >(factB));

        if (positive && out < 0) { out += static_cast< V >(b); }
     ...
     */
    template< typename V, typename U >
    V invmod(V a, U b, bool positive = false) {
        V out;
    ...
    template< typename T,
              ...
              typename U = typename std::conditional<
                  std::is_integral< T >::value && std::is_integral< V >::value ||
                      std::is_floating_point< T >::value && std::is_floating_point< V >::value,
                  typename std::conditional< sizeof(T) >= sizeof(V), T, V >::type,
                  typename std::conditional<
                      std::is_floating_point< T >::value && std::is_integral< V >::value,
                      T,
                      typename std::conditional< std::is_floating_point< V >::value &&
    ...
    T round(T a, int zeros = 0) {
        ...
        return static_cast< T >(static_cast< double64_t >(static_cast< long long int >(a * fact)) /
                                fact);
    }
     ...
     */
    inline double64_t sigmoide(double64_t in) { return (1 / (1 + std::exp(-1 * in))); }
     ...
     */
    inline int log2p(int x) { return static_cast< int >(log(x) / log(2)); }
