...
bool contains(const std::vector<T> &table, T value)
{
	...
	if (std::find(table.begin(), table.end(), value) == table.end())
	{
		return false;
	}
...
/*
	...
	*/
template <class T>
void vector2DPrint(const std::vector<std::vector<T>> values, int autosize = 0, std::ostream &os = std::cout)
{
...
/*
	...
	*/
template <class T>
void vector2DPrintInverted(const std::vector<std::vector<T>> values, int autosize = 0, std::ostream &os = std::cout)
{
	size_t maxSize = 0;
	for (size_t y = 0; y < values.size(); ++y)
	{
		if (values[y].size() > maxSize)
		{
			maxSize = values[y].size();
		}
	}
