...
namespace mcd
{
    ...
    enum TREE_SORT { ASC, DESC };

    enum TREE_DIRECTION { RIGHT, LEFT, NONE };
     ...
     */
    template< class T >
    TREE_DIRECTION treeAscSorter(std::pair< T, NodeWeight > datas,
                                 std::pair< T, NodeWeight > nodeDatas) {
     ...
     */
    template< class T >
    TREE_DIRECTION treeDescSorter(std::pair< T, NodeWeight > datas,
                                  std::pair< T, NodeWeight > nodeDatas) {
        auto w1 = datas.second;
        auto w2 = nodeDatas.second;
    ...
    class Tree {
        ...
        explicit Tree(TREE_SORT sorting = ASC) :
            _root(nullptr), _sorting(sorting), comparator(nullptr), deleter(nullptr),
            copier(nullptr) {
            this->init();
        }
        ~Tree() { freeTree(_root); }
         ...
         *
         *		\return			Return the node and the path to this node from the
         *root
         */
        std::pair< Node< T >*, std::string > find(T value) {
            if (!_root->initialized) { return std::make_pair(_root, ""); }
         ...
         *		\return			void
         */
        void applyForEach(void (*func)(Node< T >*, long int)) { this->GRD(_root, func, 0); }

        /*!
         * \brief	Return the height of the tree
         *		\return			Return the height of the tree
         */
        size_t height() { return this->height(_root); }
        ...
        void insert(std::pair< T, NodeWeight > datas) {
            ...
            while (!created) {
                ...
                } else if (this->comparator(datas, std::make_pair(node->values, node->weight)) ==
                           RIGHT) {
        ...
        void create(std::vector< std::pair< T, NodeWeight > > datas) {
            ...
            for (auto data : datas) { this->insert(data); }
        }
         ...
         */
        void add(T data, NodeWeight weight) { this->insert(std::make_pair(data, weight)); }

        /*!
         * \brief	Equilibrate the tree
         *		\param[in]		sort		Choose the sorting order with a
         *TREE_SORT
         *
         *		\return			void
         */
        void equilibrate(TREE_SORT sort = ASC) {
            switch (sort) {
                case ASC: this->rightBalance(); break;
                case DESC: this->leftBalance(); break;
            }
        }

        /*!
         * \brief	A setter for the deletion function
         *		\param[in]		func			The function to use on the
         ...
         *		\param[in]		func			The function to use to the
         *weights' comparison
         *
         *		\return			void
         */
        ...
        Tree(const Tree& other) :
            _root(nullptr), _sorting(other._sorting), comparator(nullptr), deleter(nullptr),
            copier(nullptr) {
            *this = other;
        }
        Tree operator=(const Tree& other) { this->init(); }
         ...
         */
        template< typename U >
        static std::vector< std::pair< T, NodeWeight > > convert(
            std::vector< std::pair< T, U > > in) {
        ...
        Node< T >* createRightVine(Node< T >* node) {
            while (node->left != nullptr) { node = this->rightRotate(node); }

            if (node->right != nullptr) { node->right = this->createRightVine(node->right); }
        ...
        Node< T >* createLeftVine(Node< T >* node) {
            while (node->right != nullptr) { node = this->leftRotate(node); }

            if (node->left != nullptr) { node->left = this->createLeftVine(node->left); }
         ...
         *		\param[in]		node		The root node of the tree
         *		\param[in]		func		The function to do something on each
         *node \param[in]		index		The index to let know the number of GRD call
         *
         *		\return			void
         */
         ...
         *		\param[in]		node		The root node of the tree
         *		\param[in]		index		The index to know how much point to
         *print before the node value
         *
         *		\return			void
         */
        static void printNode(Node< T >* node, long int index = -1) {
            for (long int i{0}; i < index; ++i) { std::cout << " "; }
         ...
         *		\param[in]		nodeCopy		The node where to copy the
         *datas
         *
         *		\return			void
         */
