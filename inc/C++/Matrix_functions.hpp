...
void GaussJordan(const Matrix< D >& M, Matrix< D >& I, size_t i, size_t j) {
    ...
    for (size_t index_i = 0; index_i < inside_M._i && inside_M._datas[i - 1][j - 1] == 0;
         ++index_i) {
        if (index_i == i - 1) { continue; }
...
std::ostream& operator<<(std::ostream& os, const Matrix< D >& M) {
    ...
    for (size_t i = 0; i < M._i; ++i) {
        for (size_t j = 0; j < M._j; ++j) {
            ...
            if (str_val.size() > max_length) { max_length = str_val.size(); }
        }
    }
    ...
    for (size_t i = 0; i < M._i; ++i) {
        for (size_t j = 0; j < M._j; ++j) {
            ...
            for (size_t spaces = 0; spaces < max_length - str_val.size(); ++spaces) { os << " "; }
            os << str_val;
