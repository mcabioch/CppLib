...
Matrix< C >& Matrix< C >::operator+=(const Matrix< D >& A) {
    ...
    for (size_t i = 0; i < A._i; ++i) {
        for (size_t j = 0; j < A._j; ++j) { _datas[i][j] += A._datas[i][j]; }
    }
...
Matrix< C > Matrix< C >::operator-() {
    ...
    for (size_t i = 0; i < M._i; ++i) {
        for (size_t j = 0; j < M._j; ++j) { M._datas[i][j] *= -1; }
    }
...
Matrix< C > Matrix< C >::transposed() {
    ...
    for (size_t j = 0; j < _j; ++j) {
        ...
        for (size_t i = 0; i < _i; ++i) { data.push_back(_datas[i][j]); }
...
double Matrix< C >::det() {
    ...
    if (_i == 2) {
        out = static_cast< double >(_datas[0][0]) * static_cast< double >(_datas[1][1]) -
              static_cast< double >(_datas[0][1]) * static_cast< double >(_datas[1][0]);
        return out;
    }
