...
#define THREADABLE                                                                                 \
    ...
    void launch() { Threadable::launch(this); }                                                    \
    /*!                                                                                            \
    \brief	The function call by the thread, that call update()                                     \
    \details	The update call is locked up between a lock and an unlock of the Threadable \b        \
    mutex                                                                                          \
    */                                                                                             \
    void updater() {                                                                               \
...
#define THREADABLE_LOCK std::lock_guard< mcd::mutex > guard(_mutex);
     ...
     *
     *			On use,\n
     *			- In a member function that will not be called by update(), put on the first
     *line of it : THREADABLE_LOCK
     */
    template< class C >
    class Threadable {
        public:
        ...
        Threadable() :
            m_loopThread(nullptr), m_threadStopper(false), used(false), _mutex(), sleeper(0) {}

        /*! \brief	The copy constructor of the Threadable class */
        Threadable(Threadable< C >& other) : Threadable() { *this = other; }
        /*! \brief	The move constructor of the Threadable class */
        Threadable(Threadable< C >&& other) : Threadable() { *this = std::move(other); }

        /*! \brief	The destructor of the Threadable class */
        virtual ~Threadable() { this->stop(); }
        ...
        void launch(C* obj) { m_loopThread.reset(new thread(&C::updater, obj)); }
         ...
         */
        bool launched() const { return (m_loopThread != nullptr); }

        /*!
         * \brief	Return if the thread has to stop or not
         *		\return		\b true or \b false
         */
        bool hasToStop() const { return m_threadStopper; }
        ...
        /*! \brief	Has to be set to false to stop the thread loop */
