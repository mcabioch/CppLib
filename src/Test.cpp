...
namespace mcd
{
    namespace Tests
    {
        ...
        void Test::run(const std::string& key) {
            for (auto& suite : _funcs) {
                if (key == "" || key == suite.first) {
                    ...
                    for (auto& func : suite.second) { func(); }
                }
            }
        ...
        void Test::print(std::ostream& os, size_t format) {
            ...
            sstr << _tests.size() << " tests. "
                 << (static_cast< double >(_s) / static_cast< double >(_tests.size()) * 100.0)
                 << "% success (" << _s << " success, " << _f << " failures)." << std::endl;
            os << sstr.str() << "\n";
