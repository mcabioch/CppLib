...
Gene::Gene(std::string name, GeneType type, double value, double mini, double maxi, std::vector<double> list) : _name(name),
																												_type(type),
																												_value(value),
																												_minimum(mini),
																												_maximum(maxi),
...
Gene::Gene(const Gene &g) : _name(g._name),
							_type(g._type),
							_value(g._value),
							_minimum(g._minimum),
							_maximum(g._maximum),
...
Gene Gene::copy() const
{
	...
	case GeneType::RANGED_INT:
		if (!rng(0, MutationRate - 1))
		{
			double quantity = rng(static_cast<int>(_minimum), static_cast<int>(_maximum));
	...
	case GeneType::LISTED:
		if (!rng(0, MutationRate - 1))
		{
			size_t index = static_cast<size_t>(rng(0, static_cast<int>(_list.size()) - 1));
...
void Gene::Value(const double &value)
{
	_value = value;
}
