...
namespace mcd {
	...
	int Date::weekDay()const {
		return _dAct.tm_wday;
	}

	int Date::yearDay()const {
		return _dAct.tm_yday + 1;
	}

	int Date::monthWeek()const {
		return -1;
	}
	...
	std::string Date::get(const std::string& format)const {
		...
		corres["%j"] = ((this->yearDay() < 10)?"00":((this->yearDay() < 100)?"0":"")) + tos(this->yearDay());
		...
		corres["%C"] = tos((this->year() / 100) + ((this->year() / 100 * 100 != this->year())?1:0));
		corres["%s"] = (toi(corres["%C"]) == 1)?"er":((toi(corres["%C"]) == 2)?"ème":((toi(corres["%C"]) == 3)?"ème":((toi(corres["%C"]) > 3)?"ème":"")));
