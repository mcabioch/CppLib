...
namespace mcd
{
    PathfindingDijkstra::PathfindingDijkstra(const PathfindingProblem& problem,
                                             const PathfindingNode&    start,
                                             const PathfindingNode&    end) :
        Pathfinding(problem, start, end),
        _visited(), _weights(), _computed(), _previous() {
    ...
    void PathfindingDijkstra::next() {
        try {
            ...
            if (node == _end.id) { PathfindingException::Arrived(); }
            ...
            for (auto& linkedNode : linkedNodes) {
                ...
                if ((!_computed.at(linkedNode)) || (_weights.at(linkedNode) > newWeight)) {
            ...
            this->next();
        } catch (std::out_of_range& e) { return; } catch (PathfindingException& e) {
    ...
    std::vector< size_t > PathfindingDijkstra::linked(size_t node) {
        ...
        for (auto& square : _problem.distances.at(node)) {
            if (!_visited.at(square.first)) { out.push_back(square.first); }
        }
