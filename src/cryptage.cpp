...
namespace mcd {
	...
	const uint32_t SHA2::sha2_keys[64] = {0x428a2f98, 0x71374491, 0xb5c0fbcf, 0xe9b5dba5,
	...
	void SHA2::final(uint8_t* digest){
		...
		for(int i{0}; i < 8; ++i) {
			SHA2_UNPACK32(this->h[i], &digest[i << 2]);
		}
	}
	...
	void SHA2::transform(const uint8_t* message, size_t block_nb){
		...
		for (size_t i{0}; i < block_nb; ++i) {
			...
			for (int j{0}; j < 64; ++j) {
				t1 = wv[7] + SHA256_F2(wv[4]) + SHA2_CH(wv[4], wv[5], wv[6]) + SHA2::sha2_keys[j] + w[j];
	...
	std::string MyCrypt::crypt(const std::string& str){
		...
		for(size_t i{0}; i <= str_vect.size()-i-1; ++i){
			...
			if(vect_size-i-1 != i){
				str_mixed.push_back(str_vect[i]);
			}
		}
