...
namespace mcd
{
    Fraction::Fraction() : _numerator(0), _denominator(0), _shape("") {}
    Fraction::Fraction(double value) : Fraction() { init(value); }
    Fraction::Fraction(long num, long denom) : Fraction() {
        _numerator   = num;
        _denominator = denom;
    ...
    void Fraction::init(double value) {
        ...
        if (hasPeriod(value)) {
            ...
            for (size_t i = 0; i < pos - afterVirg; ++i) { firstDix *= 10; }
            first = static_cast< long >(value * static_cast< long double >(firstDix));

            for (size_t i = 0; i < (pos + _shape.size()) - afterVirg; ++i) { secondDix *= 10; }
            ...
            for (size_t i = 0; i < number.size(); ++i) { dix *= 10; }
    ...
    void Fraction::reduice() {
        if (_denominator == 0) { throw std::string("Error Fraction : divided by 0 !"); }
    ...
    bool Fraction::hasPeriod(double value) { return (findPeriod(value) != std::string::npos); }
    ...
    Fraction& Fraction::operator%=(const Fraction& b) {
        ...
        _numerator = static_cast< long >(static_cast< double >(*this)) %
                     static_cast< long >(static_cast< double >(nb));
        _denominator = 1;
        reduice();
