...
namespace mcd
{
    ...
    void Big::equalize(std::string& val, const std::string& shape) {
        if (val.size() % 2 != 0) { val = '0' + val; }

        if (val.size() >= shape.size()) { return; }

        for (size_t i{val.size()}; i < shape.size(); ++i) { val = '0' + val; }
    ...
    void Big::set(const std::string& val) {
        ...
        for (size_t i{0}; i < inside_val.size() && inside_val[i] == ' ';) {
            inside_val = inside_val.substr(i + 1);
        }
        ...
        if (inside_val.size() == 0) {
            ...
            for (auto& c : tmp) {
                if (c >= '0' && c <= '9') { inside_val += c; }
            }

            for (size_t i{0}; i < inside_val.size() && inside_val[i] == '0';) {
                inside_val = inside_val.substr(i + 1);
            }
            if (inside_val == "") { inside_val = "0"; }
        }

        if (inside_val == "0") { _neg = false; }
    ...
    Big Big::invert() const {
        ...
        if (out != Big(0)) { out._neg = !out._neg; }
