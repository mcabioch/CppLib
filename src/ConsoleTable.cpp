...
namespace mcd {
	...
	void ConsoleTable::line(std::vector<std::string> l){
		table.push_back(l);
	}
	...
	std::ostream& operator<<(std::ostream& str, ConsoleTable& t){
		...
		/* Head */
			...
			for(size_t i{0}; i < t.table[0].size(); ++i){
				...
				for(size_t j{0}; j < width - static_cast<size_t>(width / 2); ++j){ str << t.tableCars["line"]; }
			...
			/* Body */
				...
				for(size_t j{0}; j < nbLine; ++j){
					...
					for(size_t i{1}; i < t.table.size(); ++i){
						...
						str << t.table[i][j];
						for(size_t k{0}; k < width - static_cast<size_t>(width / 2); ++k){ str << " "; }
						str << t.tableCars["col"];
					}
					str << std::endl;
				}
