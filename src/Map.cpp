...
namespace mcd
{
    ...
    Map::Map(WorldDimensions dimensions) :
        _errorTile(Tile::ERROR), _w(0), _h(0), _map(), _entities(), _printed(false) {
        setSize(dimensions);
    }
    ...
    void Map::setSize(WorldDimensions dimensions) {
        ...
        for (size_t i{0}; i < _h; ++i) {
            for (size_t j{0}; j < _w; ++j) { line.push_back(Tile::NONE); }
    ...
    void Map::setW(size_t w) { setSize(WorldDimensions{w, _h}); }

    void Map::setH(size_t h) { setSize(WorldDimensions{_w, h}); }
    ...
    bool Map::isXIn(double64_t pt, double64_t dim) {
        if (pt < 0 || pt > static_cast< double64_t >(getW()) - dim) { return false; }

        return true;
    }

    bool Map::isYIn(double64_t pt, double64_t dim) {
        if (pt < 0 || pt > static_cast< double64_t >(getH()) - dim) { return false; }
    ...
    bool Map::load(const std::string& file) {
        ...
        for (auto line : lines) {
            ...
            for (auto point : line) { mapline.push_back(static_cast< Tile >(toi(point))); }
    ...
    bool Map::save(const std::string& file) const {
        ...
        for (auto line : _map) {
            for (auto point : line) { output << tos(point); }
    ...
    bool Map::putEntity(size_t& id, double64_t x, double64_t y, double64_t w, double64_t h) {
        if (!this->isXIn(x, w) || !this->isYIn(y, h)) { return false; }
            ...
            for (size_t j{0}; j < _map[i].size(); ++j) {
                ...
                switch (_map[i][j]) {
                    case Tile::GROUND: {
                        ...
                        if (isFree(pt.top())) { neighbours.push_back(pt.top()); }
                        if (isFree(pt.left())) { neighbours.push_back(pt.left()); }
                        if (isFree(pt.bot())) { neighbours.push_back(pt.bot()); }
                        if (isFree(pt.right())) { neighbours.push_back(pt.right()); }

                        for (auto& neighbour : neighbours) {
                            switch (_map[static_cast< size_t >(neighbour.y)]
                                        [static_cast< size_t >(neighbour.x)]) {
                                case Tile::GROUND: {
                                    distances[index][array2Dto1D(
                                        neighbour,
                                        _map[static_cast< size_t >(neighbour.y)].size())] = 1;
                                } break;
                                case Tile::NONE:
                                case Tile::OBSTACLE:
                                case Tile::ERROR:
    ...
    MapMove Map::moveEntity(size_t id, Path& path, double64_t x, double64_t y) {
        ...
        if (path.next(actualNode)) {
            ...
            if (dist(actual.x, arrival.x, actual.y, arrival.y) == 0) { movement.end = true; }
        }
    ...
    bool Map::isFree(const Point& pt) {
        if (!isXIn(pt.x, 1) || !isYIn(pt.y, 1)) { return false; }
