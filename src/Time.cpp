...
namespace mcd
{
    Time::Time() : _act(), _dAct(), _milli(0), _time("") { this->init(); }

    Time::Time(const std::string& time) : _act(), _dAct(), _milli(0), _time(time) { this->reset(); }
    ...
    void Time::reset() {
        ...
        // _dAct = *(std::localtime(&_act)); Do not work anymore
        _dAct = *(&temp);
    ...
    int Time::hour() const { return _dAct.tm_hour; }

    int Time::minute() const { return _dAct.tm_min; }

    int Time::second() const { return _dAct.tm_sec; }

    int Time::milli() const { return _milli; }
    ...
    std::string Time::get(const std::string& format) const {
        ...
        corres["%N"] =
            ((this->milli() < 10) ? "00" : ((this->milli() < 100) ? "0" : "")) + tos(this->milli());
